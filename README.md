...
- **PHP 8.1 以上**（`pdo_mysql`, `mbstring`, `json`）。フレームワーク・Composer は使いません
  - さくらのレンタルサーバでは、コントロールパネルの
    「Webサイト/データ」→「スクリプト設定」→「PHP設定」でバージョンを確認・変更できます
  - 8.1 未満の場合は、その旨を表示して停止します（原因の分からない白画面にはなりません）
